...
from RemoteSensing.changedetection.models.ddpm_modules.unet import UNet
...
class STMambaSCD(nn.Module):
    def __init__(self, output_cd, output_clf, pretrained,  **kwargs):
        ...
        self.encoder = self._load_pretrained_encoder(pretrained)
    ...
    def _load_pretrained_encoder(self, pretrained_encoder_path):
        diffusion_model = diffusion.UNet()
        
        # Load pre-trained weights
        pretrained_weights = torch.load(pretrained_encoder_path)
        diffusion_model.load_state_dict(pretrained_weights)
        
        # Extract the encoder from the diffusion model
        # Assuming the encoder is the downsampling part of the UNet
